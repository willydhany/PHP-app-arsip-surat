...
if (isset($_POST['btnUpload'])) {
  ...
  # VALIDASI FORM, jika ada kotak yang kosong, buat pesan error ke dalam kotak $pesanError
	$pesanError = array();
	if (trim($mail_code)=="") {
		$pesanError[] = "Data <strong>Kode Surat</strong> tidak boleh kosong, silahkan diperbaiki !";		
	}
	if (trim($mail_to)=="") {
		$pesanError[] = "Data <b>Tujuan Surat</b> tidak boleh kosong, silahkan diperbaiki !";		
	}
	if (trim($mail_subject)=="") {
		$pesanError[] = "Data <b>Judul Surat</b> tidak boleh kosong, silahkan diperbaiki !";		
	}
	...
	if (count($pesanError)>=1 ){
	  ...
	  if (in_array($ekstensi, $ekstensi_diperbolehkan) == true || $file_upload == 0) {
	  	if ($ukuran < 2088140) {
	  		...
	  		$query = mysql_query("INSERT INTO mail_out VALUES( '$id_mail_out','$outmail_at','$mail_code','$mail_date','$mail_to','$mail_subject','$description','$file_upload','$id_mail_type','$id_user')") or die (mysql_error());
	  		if ($query) {
	  			
	  			/*header('Location:?page=Surat-Masuk-Data&pesan=input');*/
	  			echo "<meta http-equiv='refresh' content='0; url=?page=Surat-Keluar-Data&pesan=input'>";
...
?>
	<form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data" class="form-horizontal" role="form" style="margin:auto;">
	    <?php
			$carikode = mysql_query("select max(id_mail_out) from mail_out") or die (mysql_error());
			  // menjadikannya array
	    ...
	    <div class="form-group">
	      ...
	      <div class="col-sm-9">
	        <input type="text" class="form-control" name="id_mail_out" id="id_mail_out" value="<?php echo $kode_otomatis;?>" readonly>
	      </div>
	    </div>
	    <div class="form-group">
	      ...
	      <div class="col-sm-9">          
	        <div class="input-group date form_date col-sm-4" data-date="" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1" data-link-format="yyyy-mm-dd">
	                    <input class="form-control" type="text" value="<?php echo date('Y-m-d') ?>" name="outmail_at">
